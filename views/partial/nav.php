...
class NavLink
{
    public function __construct(private string $name, private string $uri)
    {
    }

    private function isCurrentUri(): bool
    {
        return $_SERVER['REQUEST_URI'] === $this->uri;
    }

    public function getATag(): string {
        $isCurrentPage = '';

        if ($this->isCurrentUri()) {
            $isCurrentPage = 'aria-current="page"';
        }

        return "<a href=\"{$this->getUri()}\" {$isCurrentPage} class=\"{$this->getClass()} rounded-md px-3 py-2 text-sm font-medium\">{$this->getName()}</a>";
    }
...
class Links
{
    ...
    public function addLink(string $name, string $uri): void
    {
        $this->links[] = new NavLink($name, $uri);
    }
...
$links = new Links();
$links->addLink('Home', '/');
$links->addLink('About', '/about');
$links->addLink('Contact', '/contact');
...
<nav class="bg-gray-800">
    ...
    <div class="space-y-1 px-2 pt-2 pb-3 sm:px-3">
        ...
        <a href="/about.php" aria-current="page" class="block rounded-md bg-gray-900 px-3 py-2 text-base font-medium text-white">About</a>
        <a href="/contact.php" aria-current="page" class="block rounded-md bg-gray-900 px-3 py-2 text-base font-medium text-white">Contact</a>
    </div>
